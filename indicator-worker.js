...
self.onmessage=({data})=>{
  const result=data.series.map(p=>{const clean=p.filter(x=>Number.isFinite(x)&&x>0);if(clean.length<60)return null;const rsiArr=rsi(clean),m=macd(clean),sma20=sma(clean,20),sma50=sma(clean,50),ema20=ema(clean,20),bands=bb(clean);return{rsiArr,...m,sma20,sma50,ema20,bb:bands}});
